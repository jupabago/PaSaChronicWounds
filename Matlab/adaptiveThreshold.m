...
binTifPath = '/Volumes/raw_data/Confocal/Carolyn/2020/Chronic wounds/Binary adaptive Images/wtd1-02/';
...
filtered1 = imbinarize (FilterImage(ch1), adaptthresh(FilterImage(ch1), 0.15));
toc
disp('filtering and stretching channel 2')
%filtered2 = imbinarize(FilterImage(ch2));
filtered2 = imbinarize (FilterImage(ch2), adaptthresh(FilterImage(ch2), 0.15));
toc
disp('filtering and stretching channel 3')
%filtered3 = imbinarize(FilterImage(ch3));
filtered3 = imbinarize (FilterImage(ch3), adaptthresh(FilterImage(ch3), 0.15));
toc
...
for slice= 1:slices
    ...
    filteredVolume(:,:,slice)= stretchedImg; %this is for the adaptive version
end
end
