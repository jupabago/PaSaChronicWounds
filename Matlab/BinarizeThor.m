...
for sampleNumber = 1:3
    for mouseNumber = 1:4
        iterateSamples(strcat('ldh_wt_',GetNum(mouseNumber),'_'),sampleNumber,.208,.520)
    end
end

for sampleNumber = 1:4
iterateSamples('wt_spxB_01_',sampleNumber,.208,.479)
end

for sampleNumber = 1:3
iterateSamples('wt_spxB_02_',sampleNumber,.132,.323)
end

for sampleNumber = 2:8
iterateSamples('wt_wt_09_',sampleNumber,.132,.369)
end

for sampleNumber = 1:4
    for mouseNumber = 5:8
iterateSamples(strcat('wt_wt_',GetNum(mouseNumber),'_'),sampleNumber,.208,.520)
    end
end

for sampleNumber = 1:3
iterateSamples('ldh_mono_01_',sampleNumber,.132,.331)
end

for sampleNumber = 1:4
iterateSamples('ldh_mono_02_',sampleNumber,.132,.365)
end

for sampleNumber = 1:3
iterateSamples('katA_mono_01_',sampleNumber,.208,.365)
end

function iterateSamples(sampleName, sampleNum,xy,z)

rawTifPathBase = '/run/user/937561/gvfs/smb-share:server=130.207.66.142,share=raw_data/Confocal/Juan/Sg-Aa/Tiff Stacks/';
binTifPathBase = '/run/user/937561/gvfs/smb-share:server=130.207.66.142,share=raw_data/Confocal/Juan/Sg-Aa/Binary Images/';
aggsFilePathBase = '/run/user/937561/gvfs/smb-share:server=130.207.66.142,share=raw_data/Confocal/Juan/Sg-Aa/Aggregate lists/'; %names are different
slicedAggsFilePathBase = '/run/user/937561/gvfs/smb-share:server=130.207.66.142,share=raw_data/Confocal/Juan/Sg-Aa/Results per slice/'; %names are different
...
resultsfilenameRed = strcat(aggsFilePath ,'_Aa.csv');
...
resultsfilenameGreen = strcat(aggsFilePath ,'_Sg.csv');
