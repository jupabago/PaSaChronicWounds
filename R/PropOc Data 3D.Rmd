...
#####
#maximum distribution isn't super representative, so for the distribution, I calculated the value at which 50% of the population is found 
...
PropOcDataFiltered1<-PropOcDataFiltered %>% group_by(Condition,TimePoint,SampleCode) %>% summarise(None = n()) %>% mutate(Sample = SampleCode)

propOcMidDensitylabeled <-merge(propOcMidDensity,PropOcDataFiltered1)
...
propOcMidDensityStats<-propOcMidDensitylabeled %>% ungroup() %>% group_by(Condition,Culture,Strain,TimePoint, Distance, Relation) %>% summarize(meanValue = mean(Value), sdValue = sd(Value), medianValue = median(Value))

ggplot()+
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Pa->Sa"),aes(y=meanValue, x = Distance,color=Condition, shape = "Mean"),size=2)+
    ...
    facet_grid(Relation ~ TimePoint)+
    scale_color_brewer(palette = "Set1")+
    theme_cowplot()+
    theme(plot.title = element_text(hjust = 0.5))

ggplot()+#Pa->Pa
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Pa->Pa"),aes(y=meanValue, x = Distance,color=Culture, shape = "Mean"),size=2)+
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Pa->Pa"),aes(y=medianValue, x = Distance,color=Culture,shape = "Median"),size=2)+
    labs(title ="mean 50 percentile Density")+
