...
PlotAllPropOcData <- function(sample ) {
  ggplot()+ 
    geom_point(data = propOcHistogramData %>% filter(Sample ==sample),aes(x=Value,y=NormDensity, color = Distance),size=2)+
    scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
...
PlotAllPropOcData("wt_d1_05")#looks cool, so let's keep going...
...
ggplot()+#linear
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Pa->Pa"),aes(y=meanValue, x = Distance,color=Condition, shape = "Mean"),size=2)+
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Pa->Pa"),aes(y=medianValue, x = Distance,color=Condition,shape = "Median"),size=2)+
...
ggplot()+#linear
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Sa->Sa"),aes(y=meanValue, x = Distance,color=Culture, shape = "Mean"),size=2)+
    geom_point(data = propOcMidDensityStats %>% filter(Relation== "Sa->Sa"),aes(y=medianValue, x = Distance,color=Culture,shape = "Median"),size=2)+
    labs(title ="mean 50 percentile Density")+
    #scale_y_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
    #scale_x_log10(breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
