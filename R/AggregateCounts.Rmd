...
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "papqsL_d4_02_Pa" & !SampleCode == "papqsL_d4_02_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "pawt_d4_05_Pa" & !SampleCode == "pawt_d4_05_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "phz_d4_06_Pa" & !SampleCode == "phz_d4_06_Sa")
...
#####  
sliceDataSum<-sliceDataMeta %>% group_by(SampleCode, Condition, Replicate, TimePoint,xySize) %>% summarise(MaxHeight = max(Height)) %>% ungroup()

ggplot()+
  geom_smooth(data = sliceDataSum %>% filter(MaxHeight>50 & MaxHeight<120) ,aes(y=NormBiomass,x=Height, color = Condition),size=2)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray")+
  theme_cowplot()+
  scale_color_brewer(palette = "Set1")+
  facet_grid(Channel~TimePoint)+
