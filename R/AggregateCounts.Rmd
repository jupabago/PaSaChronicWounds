...
PlotHistSum(HistReplicateSummary%>% filter(Culture =="co"),
            ...
            "NormalizedFrequency", "Normalized Frequency all sizes", "P. aeruginosa", "Strain")
...
```{r Plot Aggregate vs planktonics}

#Plot Frequency
PlotAggPlank<-function(rep.df,cond.df,region.df, y_var, title, species, colorStrain, facet){
ggplot()+
    geom_point(data = rep.df %>% filter(TimePoint=="Day 4")%>% filter(Species == species),
...
PlotZsliceBth<-function(){  
  ggplot()+
    geom_smooth(data = sliceDataClean,aes(y=NormBiomass,x=Height, color = Species ),size=2, alpha = .1)+
    #geom_point(data = sliceDataClean%>% filter(Species == species),aes(y=NormBiomass,x=Height, color = Region),size=1, alpha = .5)+
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray")+
    theme_cowplot()+
    scale_color_brewer(palette = "Set1")+
