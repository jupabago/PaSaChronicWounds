...
PlotHistSumCols<- function(df, df2, fill){
ggplot()+
  geom_col(data = df,aes(x=as.factor(LogBin),y=NormalizedFrequency, fill = .data[[fill]]),
           size=3,position = position_dodge( width = .9))+
  geom_point(data = df2,
                aes(x=as.factor(LogBin),y=NormalizedFrequency, fill = .data[[fill]]),
               position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75),size=1, shape = 1,show.legend = FALSE)+
...
PlotHistSumCols(HistCondRegionSum%>% filter(Condition == "Sa mono" |Condition == "wt co"|Condition == "wt mono") %>% filter(TimePoint=="Day 4"),
                  HistReplicateSummary%>% filter(Condition == "Sa mono" |Condition == "wt co"|Condition == "wt mono") %>%  
                    filter(TimePoint=="Day 4"), "Culture")#for mono vs co
PlotHistSumCols(HistCondRegionSum%>% filter(Culture == "co") %>% filter(TimePoint=="Day 4"),
                  HistReplicateSummary%>% filter(Culture == "co")%>%  filter(TimePoint=="Day 4"), "Strain")#for mono vs co
...
#this is for the defense presentation
PlotAggsPlankcols<- function(df, df2, fill){
ggplot()+
  geom_col(data = df,aes(x=Aggregation,y=AgPlNormBiomass, fill = .data[[fill]]),
           size=3,position = position_dodge( width = .9))+
  geom_point(data = df2,
                aes(x=Aggregation,y=AgPlNormBiomass, fill = .data[[fill]]),
               position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75),size=1, shape = 1,show.legend = FALSE)+
  scale_y_continuous(limits = c(0,103),expand = expand_scale(mult = c(0, 0)))+
  facet_grid(.~Species)+
  scale_fill_grey()+
...
PlotAggsPlankcols(plankAggHistCondReg%>% filter(Condition == "Sa mono" |Condition == "wt co"|Condition == "wt mono") %>% filter(TimePoint=="Day 4"),
                  plankAggHistRep%>% filter(Condition == "Sa mono" |Condition == "wt co"|Condition == "wt mono") %>%  
                    filter(TimePoint=="Day 4"), "Culture")#for mono vs co
PlotAggsPlankcols(plankAggHistCondReg%>% filter(Culture == "co") %>% filter(TimePoint=="Day 4"),
                  plankAggHistRep%>% filter(Culture == "co")%>%  filter(TimePoint=="Day 4"), "Strain")#for mono vs co
