...
##then, bad samples
aggregateList<-aggregateList %>% filter(!SampleCode == "mono_d1_03_Pa" & !SampleCode == "mono_d1_03_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "phz_d4_14_Pa" & !SampleCode == "phz_d4_14_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "pqsL_d4_02_Pa" & !SampleCode == "pqsL_d4_02_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "pqsL_d4_04_Pa" & !SampleCode == "pqsL_d4_04_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "pqsL_d4_10_Pa" & !SampleCode == "pqsL_d4_10_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "wt_d4_07_Pa" & !SampleCode == "wt_d4_07_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "papqsL_d4_02_Pa" & !SampleCode == "papqsL_d4_02_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "pawt_d4_05_Pa" & !SampleCode == "pawt_d4_05_Sa")
aggregateList<-aggregateList %>% filter(!SampleCode == "phz_d4_06_Pa" & !SampleCode == "phz_d4_06_Sa")
...
aggregateList<-aggregateList %>% mutate(Culture = recode(Condition, 'Sa mono' = "mono", 'Pa wt mono' = "mono",'pqsL mono'= "mono",'phz mono'= "mono",'wt co'= "co", 'pqsL co'= "co", 'phz co'= "co"))
aggregateList<-aggregateList%>% mutate(Strain = recode(Condition, 'Sa mono' = "Sa", 'Pa wt mono' = "wt",'pqsL mono'= "pqsL",'phz mono'= "phz",'wt co'= "wt", 'pqsL co'= "pqsL", 'phz co'= "phz"))
...
aggregateListSummaryReplicate<-aggregateList%>% group_by(Species, Strain, Culture,TimePoint, Condition, Replicate) %>% 
...
aggregateListSummary<-aggregateList %>% group_by(Species, Strain, Culture,TimePoint, Condition) %>% 
  summarise(Aggs = n(), biomass = sum(Size), meanAggSize = mean(Size), seAggSize = std.error(Size), maxSize = max(Size), minSize = min(Size)) %>%
  ungroup() %>% mutate(RatioMaxTotal = maxSize/biomass ) 
...
#Version 2
ggplot()+ 
  geom_jitter(data = aggregateListSummary, aes(x = TimePoint, y= meanAggSize, color = Strain,shape = "MeanSize"),size = 3, height = 0, width = .3)+
  geom_jitter(data = aggregateListSummary, aes(x = TimePoint, y= maxSize, color = Strain, shape = "MaxSize"),size = 3, height = 0, width = .3)+
  geom_jitter(data = aggregateListSummary, aes(x = TimePoint, y= biomass, color = Strain, shape = "TotalBiomass"),size = 3, height = 0,width= .3)+
  scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  facet_grid(Species~Culture, scales = "free", space = "free") +
  ggtitle('Mean size and largest aggregate and total biomass per condition')+
  labs(x = "",y = bquote(paste("Volume (", mu*"m"^3*")")))+
...
HistReplicate<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        ...
                        Size<= 10000  ~ 10000,
                        Size<= 100000 ~ 100000,
                        Size > 100000 ~ 1000000)) %>% group_by(Species,Culture,Strain, Condition,TimePoint, Replicate)%>% 
...
HistReplicateSummary<-HistReplicate %>% 
  group_by(Species,Culture,Strain, Condition,TimePoint, Replicate, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%
  unique()%>%ungroup() %>%  group_by(Species,Culture,Condition,TimePoint, Replicate) %>% 
...
HistCondition<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        ...
                        Size<= 10000  ~ 10000,
                        Size<= 100000 ~ 100000,
                        Size > 100000 ~ 1000000)) %>% group_by(Species,Culture,Strain, Condition,TimePoint)%>% 
  mutate(TotalBiomass = sum(Size), TotalFrequency = n()) %>% ungroup() %>%
...
HistConditionSummary<-HistCondition %>%
  group_by(Species,Culture,Strain, Condition,TimePoint,LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>% 
  unique()%>%ungroup() %>%  group_by(Species,Culture,Condition,TimePoint) %>% 
  mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))
...
ggplot()+
    geom_jitter(data = rep.df,aes(x=as.factor(LogBin),y=.data[[y_var]],  color =Condition),size=3, height = 0, width = .2, shape = 1)+
    geom_point(data = cond.df,aes(x=as.factor(LogBin),y=.data[[y_var]],  color =Condition,group = Condition),size=4, shape = 16, position = position_dodge(width = .5) )+
...
aggsHistRep<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        ...
                        Size<= 10000  ~ 10000,
                        Size<= 100000 ~ 100000,
                        Size > 100000 ~ 1000000)) %>% filter(!LogBin == 5) %>% group_by(Species,Culture,Strain,Condition,TimePoint, Replicate)%>% 
  mutate(TotalBiomass = sum(Size), TotalFrequency = n()) %>% ungroup() %>% group_by(Species,Culture,Strain,Condition,TimePoint, Replicate, LogBin)%>%
  mutate(BinBiomass = sum(Size), BinFrequency = n()) %>% ungroup() %>%
  mutate(NormalizedFrequency = BinFrequency/TotalFrequency*100, NormalizedBiomass = BinBiomass/TotalBiomass*100)
...
aggsHistCond<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        ...
                        Size<= 10000  ~ 10000,
                        Size<= 100000 ~ 100000,
                        Size > 100000 ~ 1000000)) %>% filter(!LogBin == 5) %>% group_by(Species,Culture,Strain,Condition,TimePoint)%>% 
  mutate(TotalBiomass=sum(Size),TotalFrequency=n()) %>% ungroup() %>% group_by(Species,Culture,Strain,Condition,TimePoint, LogBin)%>%
  mutate(BinBiomass = sum(Size), BinFrequency = n()) %>% ungroup() %>%
  mutate(NormalizedFrequency = BinFrequency/TotalFrequency*100, NormalizedBiomass = BinBiomass/TotalBiomass*100)

##Summary df
aggsHistCondSum<-aggsHistCond %>% group_by(Species,Culture,Strain,Condition,TimePoint, Replicate, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Culture,Strain,Condition,TimePoint, Replicate) %>% mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))%>%ungroup()
...
HistReplicate<-HistReplicate %>%  mutate(Aggregation = case_when(LogBin == 5 ~"Planktonic",LogBin>5 ~ "Aggregate")) %>%
  group_by(Species,Culture,Condition,Strain,TimePoint, Replicate, Aggregation)%>% 
...
plankAggHistRep<-HistReplicate %>% group_by(Species,Culture,Condition,Strain,TimePoint, Replicate, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Culture,Strain,Condition,TimePoint, Replicate) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))

##Normalize using Condition
HistCondition<-HistCondition %>%  mutate(Aggregation = case_when(LogBin == 5 ~"Planktonic",LogBin>5 ~ "Aggregate")) %>%
  group_by(Species,Culture,Condition,Strain, TimePoint, Aggregation)%>% 
  mutate(AgPlBiomass = sum(Size), AgPlFreq = n()) %>% ungroup() %>%
  mutate(AgPlNormFrequency = AgPlFreq/TotalFrequency*100, AgPlNormBiomass = AgPlBiomass/TotalBiomass*100)
#Collapse to only values per replicate
plankAggHistCond<-HistCondition %>% group_by(Species,Culture,Condition,Strain,TimePoint, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Culture,Strain, Condition,TimePoint) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))
...
#####  
sliceDataSum<-sliceDataMeta %>% group_by(SampleCode) %>% mutate(MaxHeight = max(Height)) %>% ungroup()

ggplot()+
  geom_smooth(data = sliceDataSum %>% filter(MaxHeight>50 & MaxHeight<120) ,aes(y=NormBiomass,x=Height, color = Strain, linetype = Culture),size=2)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray")+
  theme_cowplot()+
  scale_color_brewer(palette = "Set1")+
  facet_grid(Channel~TimePoint)+
