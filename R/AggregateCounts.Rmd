...
aggregateListSummaryReplicate<-aggregateList%>% group_by(SampleCode, Species, Strain, Culture,TimePoint, Condition, Replicate,Region) %>% 
...
aggregateListSummaryCondition<-aggregateListSummaryReplicate %>% group_by(Species, Strain, Culture,TimePoint, Condition, Region) %>% 
  summarise(meanAggs= mean(Aggs), meanBiomassLog = mean(log10(biomass)), SEBiomassLog = std.error((log10(biomass))), meanAggSize = mean(meanAggSize), seAggSize = std.error(meanAggSize))%>%  mutate(meanBiomass =  meanBiomassLog^10, SEBiomass = SEBiomassLog^10) 
...
PlotBiomassPerCondition(aggregateListSummary,"biomass")
#####
#combining all 
PlotBiomasPerSampleAndConditionWT<-function(df1,df2){
ggplot()+ 
    geom_point(data = df1, aes(x = Culture, y= meanBiomassLog,color = Species,shape = Region), 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75), size = 4)+
  geom_point(data = df2, aes(x = Culture, y= log10(biomass), color= Species, shape = Region),size = 1, 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75))+
    #scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)), limits = c(1, 100000000))+
      facet_grid(.~TimePoint, scales = "free", space = "free") +
...
PlotBiomasPerSampleAndCondition<-function(df1,df2){
ggplot()+ 
    geom_point(data = df1, aes(x = Culture, y= meanBiomassLog,color = Strain,shape = Region), 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75), size = 4)+
  geom_point(data = df2, aes(x = Culture, y= log10(biomass), color= Strain, shape = Region),size = 1, 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75))+
    #scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)), limits = c(1, 100000000))+
      facet_grid(Species~TimePoint, scales = "free", space = "free") +
  ggtitle('Biomass per sample')+
...
PlotBiomasPerSampleAndCondition(aggregateListSummaryCondition,aggregateListSummaryReplicate)
