...
ggplot()+
    geom_jitter(data = HistReplicateSummary,aes(x=as.factor(LogBin),y=.data[[y_var]],  color =Species ),size=3, height = 0, width = .2, shape = 1)+
    geom_point(data = HistConditionSummary,aes(x=as.factor(LogBin),y=.data[[y_var]],  color =Species,group = Species),size=4, shape = 16, position = position_dodge(width = .3) )+
...
ggplot()+
    ...
    scale_colour_grey()+
    theme(plot.title = element_text(hjust = 0.5)) }
PlotHistRepBioTrend(HistReplicateSummary, "Pa")
PlotHistRepBioTrend(HistReplicateSummary, "Sa")
...
##Summary df
aggsHistCondSum<-aggsHistCond %>% group_by(Species,Condition,TimePoint, Replicate, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint, Replicate) %>% mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))
...
PlotHistRepBioTrend(aggsHistRepSum, "Pa")
PlotHistRepBioTrend(aggsHistRepSum, "Sa")
...
#Plot Frequency
ggplot()+
  geom_jitter(data = plankAggHistRep,aes(x=as.factor(Aggregation),y=AgPlNormFrequency,  color =Species, ),size=3, height = 0, width = .2, shape = 1)+ 
  geom_point(data = plankAggHistCond,aes(x=as.factor(Aggregation),y=AgPlNormFrequency,  color =Species,group = Species),size=4, shape = 16, position = position_dodge(width = .3) )+
  facet_grid(Condition~TimePoint)+
...
ggplot()+
  ...
  geom_jitter(data = plankAggHistCond,aes(x=as.factor(Aggregation),y=AgPlNormBiomass,  color =Condition,group = Species),size=5, shape = 16,height = 0, width = .2) +
    facet_grid(Species~TimePoint)+
  ggtitle("Normalized Biomass (%) Aggs vs Plank")+
    labs(y = "Normalized Biomass (%)",x = bquote(paste("Aggregate Volume (", mu*"m"^3*")")))+  scale_color_manual( values = c( "red","blue","green", "gray"),labels = c("Sa Mono", "phz co", "pqsL co", "wt co"))+
    theme_cowplot()+
    panel_border() +
    theme(plot.title = element_text(hjust = 0.5)) 
#####
