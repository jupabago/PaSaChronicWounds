...
aggregateListSummaryCondition<-aggregateListSummaryReplicate %>% group_by(Species, SaStrain,Strain, Culture,TimePoint, Condition, Region) %>% 
  summarise(meanAggs= mean(Aggs), meanBiomassLog = mean(log10(biomass)), SEBiomassLog = std.error((log10(biomass))), meanAggSize = mean(meanAggSize), seAggSize = std.error(meanAggSize))%>%  mutate(meanBiomass =  meanBiomassLog^10, SEBiomass = SEBiomassLog^10) 
...
PlotBiomasPerSampleAndCondition<-function(df1,df2, df3, species){
ggplot()+ 
    geom_point(data = df1 %>% filter(TimePoint=="Day 4") %>% filter(Species==species), aes(x = SaStrain, y= meanBiomassLog,shape = Region), 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75), size = 4)+
  geom_point(data = df2%>% filter(TimePoint=="Day 4") %>% filter(Species==species), aes(x = SaStrain, y= log10(biomass), shape = Region),size = 1, 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75))+ 
  geom_point(data = df3 %>% filter(TimePoint=="Day 4") %>% filter(Species==species), aes(x = SaStrain, y= meanBiomassLog), shape = 3, size = 4)+
    #scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)), limits = c(1, 100000000))+
      facet_grid(.~Species, scales = "free", space = "free") +
...
PlotBiomasPerSampleAndCondition(aggregateListSummaryCondition,
                                aggregateListSummaryReplicate,
                                aggregateListSummaryCondNoRegion, "S. aureus")
...
PlotMeanAggsPerSampleAndCondition<-function(df1,df2, df3,species){
ggplot()+ 
    geom_point(data = df1 %>% filter(TimePoint=="Day 4") %>% filter(Species == species), aes(x = SaStrain, y= meanAggs,shape = Region), 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75), size = 4)+
  geom_point(data = df2%>% filter(TimePoint=="Day 4")%>% filter(Species == species), aes(x = SaStrain, y= Aggs, shape = Region),size = 1, 
             position = position_jitterdodge(jitter.height = 0, jitter.width = .3,dodge.width = 0.75))+
    geom_point(data = df3 %>% filter(TimePoint=="Day 4")%>% filter(Species == species), aes(x = SaStrain, y= meanAggs),shape = 3,size = 4)+
    scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)), limits = c(10, 100000))+
...
PlotMeanAggsPerSampleAndCondition(aggregateListSummaryCondition,
                                aggregateListSummaryReplicate, 
                                aggregateListSummaryCondNoRegion, "S. aureus")
...
PlotBiomasPerSampleAndCondition(aggregateListSummaryCondition,
                                aggregateListSummaryReplicate,
                                aggregateListSummaryCondNoRegion, "P. aeruginosa")
...
PlotMeanAggsPerSampleAndConditionPa(aggregateListSummaryCondition,
                                aggregateListSummaryReplicate, 
                                aggregateListSummaryCondNoRegion, "P. aeruginosa")
```
...
HistReplicateSummary<-HistReplicate %>% 
  group_by(Species,Culture,SaStrain, Strain,Region,Condition,TimePoint, Replicate,SampleCode, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%
  unique()%>%ungroup() %>%  group_by(Species,Strain, SaStrain,Culture,Condition,TimePoint, Replicate, SampleCode) %>% 
...
HistConditionSummary<-HistCondition %>%
  group_by(Species,Culture,SaStrain, Strain, Condition,TimePoint,LogBin,NormalizedFrequency,NormalizedBiomass) %>% 
  summarize() %>% unique()%>%ungroup() %>%
  group_by(Species,Culture,Strain,SaStrain, Condition,TimePoint) %>% 
  mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))
#####
...
HistCondRegionSum<-HistConditionRegin %>%
  group_by(Species,Culture,SaStrain, Strain, Condition,TimePoint, Region,LogBin,NormalizedFrequency,NormalizedBiomass) %>% 
  summarize() %>% unique()%>%ungroup() %>%
  group_by(Species,Culture,Strain,SaStrain, Condition,TimePoint, Region) %>% 
  mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))
#####
...
PlotHistSum<-function(rep.df,cond.df,region.df, y_var, title, species, colorStrain){
ggplot()+
...
####---Figure 1C
PlotHistSum(HistReplicateSummary %>% filter(SaStrain %in% c("Sa mono", "co-wt")),
            HistConditionSummary%>% filter(SaStrain %in% c("Sa mono", "co-wt")),
            HistCondRegionSum%>% filter(SaStrain %in% c("Sa mono", "co-wt")),
            "NormalizedFrequency", "Normalized Frequency all sizes", "S. aureus", "Culture")

PlotHistSum(HistReplicateSummary %>% filter(Strain %in% c("wt")),
            HistConditionSummary%>% filter(Strain %in% c("wt")),
            HistCondRegionSum%>% filter(Strain %in% c("wt")),
            "NormalizedFrequency", "Normalized Frequency all sizes", "P. aeruginosa", "Culture")

PlotHistSum(HistReplicateSummary,
            HistConditionSummary,
            HistCondRegionSum,
            "NormalizedFrequency", "Normalized Frequency all sizes", "S. aureus", "SaStrain")

PlotHistSum(HistReplicateSummary,HistCondRegionSum,HistConditionSummary,"NormalizedBiomass", "Normalized Biomass all sizes")
...
plankAggHistRep<-HistReplicate %>% 
  group_by(Species,Culture,Condition,Strain,TimePoint, Replicate, SaStrain, Region, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>% 
  summarize() %>%  unique()%>%ungroup() %>%  
  group_by(Species,Culture,Condition,Strain,TimePoint, Replicate, SaStrain, Region) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))
...
HistConditionRegin<-HistConditionRegin %>%  mutate(Aggregation = if_else(LogBin == "(.5, 5]", "Planktonic", "Aggregate")) %>%
  group_by(Species,Culture,Condition,Strain, TimePoint,Region, SaStrain, Aggregation)%>% 
  mutate(AgPlBiomass = sum(Size), AgPlFreq = n()) %>% ungroup() %>%
  mutate(AgPlNormFrequency = AgPlFreq/TotalFrequency*100, AgPlNormBiomass = AgPlBiomass/TotalBiomass*100)
#Collapse to only values per replicate
plankAggHistCondReg<-HistConditionRegin %>% 
  group_by(Species,Culture,Condition,SaStrain, Region,Strain,TimePoint, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>%
  summarize() %>%  unique()%>%ungroup() %>%
  group_by(Species,Culture,Condition,SaStrain, Region,Strain,TimePoint) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))

##Normalize using Condition 
HistCondition<-HistCondition %>%  mutate(Aggregation = if_else(LogBin == "(.5, 5]", "Planktonic", "Aggregate")) %>%
  group_by(Species,Culture,Condition,SaStrain, TimePoint,Strain, Aggregation)%>% 
  mutate(AgPlBiomass = sum(Size), AgPlFreq = n()) %>% ungroup() %>%
  mutate(AgPlNormFrequency = AgPlFreq/TotalFrequency*100, AgPlNormBiomass = AgPlBiomass/TotalBiomass*100)
#Collapse to only values per replicate
plankAggHistCond<-HistCondition %>% 
  group_by(Species,Culture,Condition,Strain,SaStrain,TimePoint, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>%
  summarize() %>%  unique()%>%ungroup() %>%  
  group_by(Species,Culture,Condition,Strain,SaStrain,TimePoint) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))


#Plot Frequency
PlotAggPlank<-function(rep.df,cond.df,region.df, y_var, title, species, colorStrain){
ggplot()+
    geom_point(data = rep.df %>% filter(TimePoint=="Day 4")%>% filter(Species == species),
...
###---Figure 1D
PlotAggPlank(plankAggHistRep%>% filter(SaStrain %in% c("Sa mono", "co-wt")),
             plankAggHistCond%>% filter(SaStrain %in% c("Sa mono", "co-wt")),
             plankAggHistCondReg%>% filter(SaStrain %in% c("Sa mono", "co-wt")),
             "AgPlNormBiomass", "Normalized Biomass", "S. aureus", "Culture")

PlotAggPlank(plankAggHistRep,
             plankAggHistCond,
             plankAggHistCondReg,
             "AgPlNormBiomass", "Normalized Biomass", "S. aureus", "SaStrain")
