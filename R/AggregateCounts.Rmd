...
sliceDataMeta<-sliceDataMeta %>% mutate(Height = Zdim*Slice)
sliceDataMeta<-sliceDataMeta %>% group_by(SampleCode, Channel) %>% mutate(TotalBiomass = sum(Biomass)) %>% ungroup() %>% mutate(NormBiomass = Biomass/TotalBiomass*100)


ggplot()+
  geom_point(data = sliceDataMeta,aes(x=NormBiomass,y=Height, color = Condition, shape = TimePoint ),size=2)+
