...
removable <- aggregateList %>% filter(Species == "Pa" & Condition == "mono")
aggregateList <- anti_join(aggregateList, removable)
##bad samples
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "mono_d1_03_Pa" & !SampleCode == "mono_d1_03_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "phz_d4_14_Pa" & !SampleCode == "phz_d4_14_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "pqsL_d4_02_Pa" & !SampleCode == "pqsL_d4_02_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "pqsL_d4_04_Pa" & !SampleCode == "pqsL_d4_04_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "pqsL_d4_10_Pa" & !SampleCode == "pqsL_d4_10_Sa")
aggregateListRemoved<-aggregateList %>% filter(!SampleCode == "wt_d4_07_Pa" & !SampleCode == "wt_d4_07_Sa")
...
aggregateList$Condition <- factor(aggregateList$Condition, levels = c("mono", "wt", "pqsL", "phz"),labels = c('Sa mono','wt co', 'pqsL co', 'phz co'))
aggregateList$TimePoint <- factor(aggregateList$TimePoint, levels = c("d1", "d4"),labels = c('Day 1','Day 4'))
#agregate input functions
ReadAggregateFile<-function(file, species, timepoint, condition, replicate, sampleCode){#make a dataframe of the aggregate size list and add experimental parameters
  print(file)
...
GetAggregates<-function(){#import all experiments at once
   ...
   for (aggList in 1:length(fileList)){
     ...
     growthDf<-rbind(growthDf,ReadAggregateFile(fileList[aggList],attrList[4],attrList[2],attrList[1],attrList[3], fileName))
     }
   return(growthDf)
}
```

```{r aggregate wrangling and overview plots}
...
ggplot()+ 
  geom_jitter(data = aggregateListSummaryReplicate, aes(x = TimePoint, y= meanAggSize, color = Condition,shape = "MeanSize"),size = 3, height = 0, width = .3)+
  geom_jitter(data = aggregateListSummaryReplicate, aes(x = TimePoint, y= maxSize, color = Condition, shape = "MaxSize"),size = 3, height = 0, width = .3)+
  geom_jitter(data = aggregateListSummaryReplicate, aes(x = TimePoint, y= biomass, color = Condition, shape = "TotalBiomass"),size = 3, height = 0,width= .3)+
...
#####
ggplot()+ 
  geom_jitter(data = aggregateListSummary, aes(x = TimePoint, y= meanAggSize, color = Condition,shape = "MeanSize"),size = 3, height = 0, width = .3)+
  geom_jitter(data = aggregateListSummary, aes(x = TimePoint, y= maxSize, color = Condition, shape = "MaxSize"),size = 3, height = 0, width = .3)+
  geom_jitter(data = aggregateListSummary, aes(x = TimePoint, y= biomass, color = Condition, shape = "TotalBiomass"),size = 3, height = 0,width= .3)+
  scale_y_log10( breaks = scales::trans_breaks("log10", function(x) 10^x), labels = scales::trans_format("log10", scales::math_format(10^.x)))+
  facet_grid(. ~ Species, scales = "free", space = "free") +
  ggtitle('Mean size and largest aggregate and total biomass per condition')+
  labs(x = "",y = bquote(paste("Volume (", mu*"m"^3*")")))+
  scale_shape_manual(name="Metric",values=c(MeanSize=1, MaxSize=2, TotalBiomass=17))+
  scale_color_brewer(palette = "Set1")+
  theme_cowplot()+
...
#First per replicate:
HistReplicate<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        Size<= 10 ~ 10,
                        Size<= 100    ~ 100,
                        Size<= 1000   ~ 1000,
                        Size<= 10000  ~ 10000,
...
HistReplicateSummary<-HistReplicate %>% group_by(Species,Condition,TimePoint, Replicate, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint, Replicate) %>% mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))

#Now per condition. The difference is int the "group_by" part
HistCondition<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        Size<= 10 ~ 10,
                        Size<= 100    ~ 100,
                        Size<= 1000   ~ 1000,
                        Size<= 10000  ~ 10000,
                        Size<= 100000 ~ 100000,
                        Size > 100000 ~ 1000000)) %>% group_by(Species,Condition,TimePoint)%>% 
...
HistConditionSummary<-HistCondition %>%
  group_by(Species,Condition,TimePoint,LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>% 
  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint) %>% 
...
PlotHistSum<-function(rep.df,cond.df, y_var, title){
ggplot()+
    geom_jitter(data = HistReplicateSummary,aes(x=as.factor(LogBin),y=.data[[y_var]],  color =Condition ),size=3, height = 0, width = .2, shape = 1)+
    geom_point(data = HistConditionSummary,aes(x=as.factor(LogBin),y=.data[[y_var]],  color =Condition,group = Condition),size=4, shape = 16, position = position_dodge(width = .5) )+
    scale_y_log10(breaks = c(100, 10, 1, .1, .01), labels = c(100, 10, 1, .1, .01))+
    facet_grid(Species~TimePoint)+
    ggtitle(title)+
    labs(y = paste(y_var,"(%)"),x = bquote(paste("Aggregate Volume (", mu*"m"^3*")")))+
    scale_color_brewer(palette = "Set1")+
    theme_cowplot()+
    panel_border() +
    theme(plot.title = element_text(hjust = 0.5), 
...
PlotHistSum(HistReplicateSummary,HistConditionSummary,"NormalizedFrequency", "Normalized Frequency all sizes")

PlotHistSum(HistReplicateSummary,HistConditionSummary,"NormalizedBiomass", "Normalized Biomass all sizes")
...
##Summary df
aggsHistRepSum<-aggsHistRep %>% group_by(Species,Condition,TimePoint, Replicate, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint, Replicate) %>% mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))
...
aggsHistCond<-aggregateList %>% mutate(LogBin = case_when(Size <= 5 ~ 5,
                        ...
                        Size<= 10000  ~ 10000,
                        Size<= 100000 ~ 100000,
                        Size > 100000 ~ 1000000)) %>% filter(!LogBin == 5) %>% group_by(Species,Condition,TimePoint)%>% 
  mutate(TotalBiomass=sum(Size),TotalFrequency=n()) %>% ungroup() %>% group_by(Species,Condition,TimePoint, LogBin)%>%
  mutate(BinBiomass = sum(Size), BinFrequency = n()) %>% ungroup() %>%
  mutate(NormalizedFrequency = BinFrequency/TotalFrequency*100, NormalizedBiomass = BinBiomass/TotalBiomass*100)

##Summary df
aggsHistCondSum<-aggsHistCond %>% group_by(Species,Condition,TimePoint, Replicate, LogBin,NormalizedFrequency,NormalizedBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint, Replicate) %>% mutate(checkSumFreq =sum(NormalizedFrequency), checkSumBiomass = sum(NormalizedBiomass))%>%ungroup()
...
HistReplicate<-HistReplicate %>%  mutate(Aggregation = case_when(LogBin == 5 ~"Planktonic",LogBin>5 ~ "Aggregate")) %>%
...
plankAggHistRep<-HistReplicate %>% group_by(Species,Condition,TimePoint, Replicate, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint, Replicate) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))

##Normalize using Condition
HistCondition<-HistCondition %>%  mutate(Aggregation = case_when(LogBin == 5 ~"Planktonic",LogBin>5 ~ "Aggregate")) %>%
  group_by(Species,Condition,TimePoint, Aggregation)%>% 
  mutate(AgPlBiomass = sum(Size), AgPlFreq = n()) %>% ungroup() %>%
  mutate(AgPlNormFrequency = AgPlFreq/TotalFrequency*100, AgPlNormBiomass = AgPlBiomass/TotalBiomass*100)
#Collapse to only values per replicate
plankAggHistCond<-HistCondition %>% group_by(Species,Condition,TimePoint, Aggregation,AgPlNormFrequency,AgPlNormBiomass) %>% summarize() %>%  unique()%>%ungroup() %>%  group_by(Species,Condition,TimePoint) %>% mutate(checkAggFreq =sum(AgPlNormFrequency), checkAggBiomass = sum(AgPlNormBiomass))

#Plot Frequency
ggplot()+
  geom_jitter(data = plankAggHistRep,aes(x=as.factor(Aggregation),y=AgPlNormFrequency,  color =Condition ),size=3, height = 0, width = .2, shape = 1)+ 
  geom_point(data = plankAggHistCond,aes(x=as.factor(Aggregation),y=AgPlNormFrequency,  color =Condition,group = Condition),size=4, shape = 16, position = position_dodge(width = .3) )+
  facet_grid(Species~TimePoint)+
  ggtitle("Normalized Frequency (%) Aggs vs Plank")+
...
#Plot Biomass
ggplot()+
  geom_jitter(data = plankAggHistRep,aes(x=as.factor(Aggregation),y=AgPlNormBiomass,  color =Condition ),size=2, height = 0, width = .2, shape = 1)+
  geom_point(data = plankAggHistCond,aes(x=as.factor(Aggregation),y=AgPlNormBiomass,  color =Condition),size=5, shape = 16, position = position_dodge2(width = .5)) +
  facet_grid(Species~TimePoint)+
...
plankAggHistSummary<-plankAggHistRep %>% group_by(Species,TimePoint, Condition,Aggregation) %>%
...
ggplot()+
  geom_col(data = plankAggHistSummary %>% filter(Aggregation=="Aggregate"),aes(x=Aggregation,y=MeanBiomass, fill = Condition),
           size=3,position = position_dodge( width = .9))+
  geom_jitter(data = plankAggHistRep%>% filter(Aggregation=="Aggregate"),aes(x=Aggregation,y=AgPlNormBiomass, group = Condition),shape = 1,size=2,position = position_dodge(width = .9),show.legend = FALSE)+
  geom_errorbar(data = plankAggHistSummary%>% filter(Aggregation=="Aggregate"),aes(x=Aggregation,ymin=(MeanBiomass-SEBiomass), ymax = (MeanBiomass+SEBiomass), group = Condition),width = 0, size=.5,position = position_dodge(width = .9))+
  ...
  theme(plot.title = element_text(hjust = 0.5),strip.text = element_text(size = 9, face = "italic",margin = margin(.1,0,.1,0, "cm")),
        ...
        legend.position = c(.45, 0.60))
#####
#Normalized Frequency and Biomass contribution of Aggregate bins
#####
#Collapse to data per condition
AggsOnlySummary<-aggsHistRepSum %>% group_by(Species,TimePoint, Condition,LogBin) %>%
...
ggplot()+
    geom_col(data = AggsOnlySummary, aes(x=as.factor(LogBin),y=MeanFreq,  fill =Condition ),size=4,position = position_dodge( width = .9))+
    geom_jitter(data = aggsHistRepSum, aes(x=as.factor(LogBin),y=NormalizedFrequency, group = Condition),shape = 1,size=2,position = position_dodge( width = .9))+
    geom_errorbar(data = AggsOnlySummary,aes(x=as.factor(LogBin),ymin=(MeanFreq-SEFreq), ymax = (MeanFreq+SEFreq), group = Condition),width = 0, size= .5,position = position_dodge( width = .9))+
    facet_grid(TimePoint~Species)+
...
AggsOnlySummaryRep<-aggsHistRepSum %>% group_by(Species,TimePoint, Replicate, Condition,LogBin) %>%
  summarise(MeanFreq = mean(NormalizedFrequency), SEFreq = std.error(NormalizedFrequency), MeanBiomass = mean(NormalizedBiomass),
            SEBiomass = std.error(NormalizedBiomass))
ggplot()+
    geom_col(data = AggsOnlySummary, aes(x=as.factor(LogBin),y=MeanBiomass,  fill =Condition ),size=4,position = position_dodge( width = .9))+
    ...
    geom_errorbar(data = AggsOnlySummary,aes(x=as.factor(LogBin),ymin=(MeanBiomass-SEBiomass), ymax = (MeanBiomass+SEBiomass), group = Condition),width = 0, size= .5,position = position_dodge( width = .9))+
    facet_grid(TimePoint~Species)+
...
#get metadata
metaData.df <-read.csv('/Volumes/raw_data/Confocal/Carolyn/2020/Chronic wounds/image data copy.csv', header = TRUE)
#merge files
metaData.df<-metaData.df %>% select(SampleCode, mouse, Position, size, Voxel.size,Region, Zdim)
sliceDataMeta<-metaData.df %>%  merge(slicedData, by = "SampleCode")
#clean up
remove <- sliceDataMeta %>% filter(Channel == 2 & Condition == "mono")
sliceDataMeta <- anti_join(sliceDataMeta, remove)
#rename variables
sliceDataMeta$Channel <- factor(sliceDataMeta$Channel, levels = c(1, 2, 3),labels = c('S. aureus','P. aeruginosa','host'))
sliceDataMeta$Condition<- factor(sliceDataMeta$Condition, levels = c("mono", "wt", "pqsL", "phz"),labels = c('Sa mono','wt co', 'pqsL co', 'phz co'))
sliceDataMeta$TimePoint <- factor(sliceDataMeta$TimePoint, levels = c("d1", "d4"),labels = c('Day 1','Day 4'))
#calculate relative biomass
sliceDataMeta<-sliceDataMeta%>% filter(!Channel == "host") %>% mutate(Height = Zdim*Slice) %>% group_by(SampleCode, Channel) %>% mutate(TotalBiomass = sum(Biomass)) %>% ungroup() %>% mutate(NormBiomass = Biomass/TotalBiomass*100) 
...
#####
#Max slice?
sliceDataMeta<-sliceDataMeta %>% group_by(SampleCode, Condition, Replicate, TimePoint) %>% mutate(MaxHeight = max(Height))
ggplot()+
  geom_point(data = maxSlice%>% filter(MaxHeight<120 & MaxHeight>50),aes(x=Replicate,y=MaxHeight, color = Condition),size=4)+
facet_grid(.~TimePoint, scales = "free", space = "free")+
  scale_color_brewer(palette = "Set1")+
  geom_hline(yintercept = 120)+
  geom_hline(yintercept = 50)+
  theme_cowplot()+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(angle = 70, hjust = 1))

ggplot()+
  #geom_point(data = sliceDataMeta ,aes(y=NormBiomass,x=Height, color = Condition, group = Replicate),size=.5)+
  geom_smooth(data = sliceDataMeta %>% filter(MaxHeight>50 & MaxHeight<120) ,aes(y=NormBiomass,x=Height, color = Condition),size=2)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray")+
  theme_cowplot()+
  scale_color_brewer(palette = "Set1")+
  facet_grid(Channel~TimePoint)+
